...
public class SignUpNextActivity extends AppCompatActivity implements View.OnFocusChangeListener {
    ...
    public CustomRegularEditView InsCedula;
    int i = 0;
    ...
    CheckBox checkBox;
    AbstractClient serv;
    boolean usernameCheck = true,usermobCheck=false, emailcheck = true, termsAccept = false, referralCodeCheck = true;
    private Object JsonRawValue;
    ...
    public void onViewClicked2(View view) {
        switch (view.getId()) {
            case R.id.rigister_btn2:
                ...
                if (recibirN1.getText().toString().equals(generarN1.getText().toString())) {
                    ...
                    Toast toast1 = Toast.makeText(getApplicationContext(), "!!Error Vuelva a generar un codigo muchas gracias!!", Toast.LENGTH_SHORT);
                    toast1.show();
                }
    ...
    ////// validar los campos de verificacion///////
    ...
    protected void verifyTelefono() {
        new Thread() {
            public void run() {
                try {
                    String url = Const.ServiceType.VERIFICAR_TELEFONO;
                    AbstractClient serv = new AbstractClient();
                    ...
                    if (status.equals(true)) {
                        TextView test = (TextView) findViewById(R.id.textView3);
                        test.setText("El telefono ya se encuentra registrado revisar por favor");
                        userMobileNumber.setBackgroundColor(ContextCompat.getColor(getApplicationContext(), R.color.bt_light_gray));
                        //usermobCheck=false;
                    } else {
                        userMobileNumber.setBackgroundColor(ContextCompat.getColor(getApplicationContext(), R.color.colorAccent));
                        //usermobCheck=true;
                    }
                } catch (Exception e) {
                    usernameCheck = false;
    ...
    try {
            ...
            if (status.equals(true)) {
                TextView test = (TextView) findViewById(R.id.textView3);
                test.setText("Telefono ya registrado por favor cambie de numero");
                userMobileNumber.setError("Telefono registrado");
                userMobileNumber.requestFocus();
            }
        } catch (Exception e) {
            usernameCheck = false;
...
*/
    ...
    protected void verifyReferal() {
        new Thread() {
            public void run() {
                try {
                    String url = Const.ServiceType.APPLY_REFERRAL;
                    AbstractClient serv = new AbstractClient();
                    String response = serv.web(url, "username=" + userReferralCode.getText().toString());
                    System.out.println("response register: " + response);
                    JSONObject obj = new JSONObject(response);
                    Boolean status = obj.getBoolean("existe");
                    if (status.equals(true)) {
                        ...
                        userReferralCode.setBackgroundColor(ContextCompat.getColor(getApplicationContext(), R.color.bt_white));
                        //userReferralCode.requestFocus();
                    }
                } catch (Exception e) {
                    usernameCheck = false;
